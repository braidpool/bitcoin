...
{
    argsman.AddArg("-chain=<chain>", "Use the chain <chain> (default: main). Allowed values: main, test, signet, regtest, cpunet", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
    ...
    argsman.AddArg("-cpunet", "Use the CPU-mined test chain. Equivalent to -chain=cpunet.", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const ChainType chain)
{
    ...
    case ChainType::CPUNET:
        return std::make_unique<CBaseChainParams>("cpunet", 28332, 28334);
    }
    assert(false);
}
